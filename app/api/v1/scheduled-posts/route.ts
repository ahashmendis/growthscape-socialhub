...
export async function GET(request: NextRequest) {
  try {
    ...
    const posts = await schedulerService.listScheduledPosts(brandId, dateFrom, dateTo);
    return successResponse(posts);
  } catch (error) {
    if (isAppError(error)) {
      return errorResponse(error.code as ErrorCode, error.message, 401);
    }
    return errorResponse("INTERNAL_ERROR", "Failed to list scheduled posts", 500);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = await import("@/features/scheduler/schemas/draft").then((m) =>
      m.scheduleDraftSchema.safeParse(body)
    );
    ...
    const post = await schedulerService.scheduleDraft(validation.data);
    return successResponse(post);
  } catch (error) {
    if (isAppError(error)) {
      return errorResponse(error.code as ErrorCode, error.message, 400);
    }
    return errorResponse("INTERNAL_ERROR", "Failed to schedule post", 500);
  }
}
